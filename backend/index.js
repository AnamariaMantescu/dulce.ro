...
async function checkTimeSync() {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      if (diffSeconds > 30) {
        console.warn(`⚠️ WARNING: Time difference of ${diffSeconds} seconds detected!`);
        ...
        console.log("✅ Time is sufficiently synchronized");
      }
    ...
    req.on('error', (e) => {
      console.error("❌ Time sync check failed:", e.message);
      reject(e);
    });
  ...
  try {
    ...
    try {
      if (!admin.apps.length) {
        ...
        console.log("✅ Firebase Admin initialized with credential file");
      }
    } catch (error) {
